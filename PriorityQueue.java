...
class PriorityQueue<E, P> {
    ...
    public Node add(E e, P priority) {
        Node node = new Node(e, priority, tree.size()); //adds a new node
        tree.add(node); //adds the new node created aove into the tree
        pullUp(tree.size()-1); // maintain the heap
        return node;
    }
    ...
    public boolean contains(E e) {
        ...
        for(int i = 0; i < tree.size(); i++){ // treaverse the tree
            Node node = tree.get(i);
            if(node != null && node.value().equals(e)){// checking to see if the node has the same value as e
                contain = true;
            }
        }
